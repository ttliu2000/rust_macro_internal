...
fn get_hook_trait_name(item:&ItemStruct) -> Ident {
    format_ident!("I{}_StateMachineHook", item.ident)
}
    ...
    state_graph.stmts.iter().filter_map(|x| x.get_state_sequence_statement()).map(|stmt| {
        ...
        quote! {
            (#state_name_type::#start_state, #event_name_type::#event_name) => {
                    self.hook.as_mut().map(|h| h.on_state_enter(& #state_name_type::#start_state, & self.current_state));
                    self.hook.as_mut().map(|h| h.on_event_triggered(& self.current_state, event));
                    self.current_state = #state_name_type::#end_state;
                    self.hook.as_mut().map(|h| h.on_state_exit(& #state_name_type::#start_state, & self.current_state));
                }
        }
    }).collect::<Vec<_>>()
}
...
fn create_hook_trait(item:&ItemStruct) -> TokenStream {
    let trait_name = get_hook_trait_name(item);
    let state_type_name = get_state_type_name(item);
    let event_type_name = get_trigger_event_type_name(item);
    quote! {
        pub trait #trait_name {
            fn on_state_enter(&mut self, pre: & #state_type_name, state: & #state_type_name) {}
            fn on_state_exit(&mut self, pre: & #state_type_name, state: & #state_type_name) {}
            fn on_event_triggered(&mut self, pre: & #state_type_name, event: & #event_type_name) {}
        }
    }
}

pub fn expand_with_trait(attr: InitArgs, mut item: ItemStruct) -> TokenStream {
    ...
    let hook_trait_name = get_hook_trait_name(&item);
    match push_named_field(&mut item, syn::parse_quote! {
        hook: Option<Box<dyn #hook_trait_name>>
    }) {
        Ok(_) => {}
        Err(e) => {
            return e;
    ...
    match parse_state_from_file(&file_path.value()) {
        Ok(state_graph) => {
            ...
            let hook_trait = create_hook_trait(&item);
